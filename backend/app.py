...
def get_request_session_id():
    """Read guest session identifier from request headers."""
...
def login():
    ...
    try:
        ...
        # Migrate guest session data to this account on first login from this device/session
        session_id = get_request_session_id()
        migrated = {'history_migrated': 0, 'saved_migrated': 0}
        if session_id:
            migrated = db_handler.assign_guest_data_to_user(user_data['id'], session_id)
...
def get_history():
    ...
    try:
        ...
        history = db_handler.get_search_history(
            user_id=user_id,
            limit=limit,
            session_id=session_id if user_id is None else None
        )
...
def get_saved_recommendations():
    ...
    try:
        ...
        saved = db_handler.get_saved_recommendations(
            user_id=user_id,
            limit=limit,
            session_id=session_id if user_id is None else None
        )
...
def delete_history(history_id):
    ...
    try:
        ...
        deleted = db_handler.delete_search_history(
            history_id,
            user_id=user_id,
            session_id=session_id if user_id is None else None
        )
...
def delete_saved(saved_id):
    ...
    try:
        ...
        deleted = db_handler.delete_saved_recommendation(
            saved_id,
            user_id=user_id,
            session_id=session_id if user_id is None else None
        )
